...
- Mix informational, comparison, and decision-stage queries`,
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden text-th-text" style={{ background: "var(--bg-deep)" }}>
      {/* ── Rate limit modal ───────────────────────────────── */}
      ...
      <aside
        className={`
          fixed inset-y-0 left-0 z-30 flex w-[252px] shrink-0 flex-col border-r border-[var(--border-subtle)] bg-[var(--bg-elevated)]
      ...
      >
        ...
        <div className="border-b border-[var(--border-subtle)] px-4 py-3">
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <button
              onClick={() => setMobileSidebarOpen(false)}
              className="ml-auto flex md:hidden items-center justify-center w-7 h-7 rounded-md text-[var(--text-disabled)] hover:text-white hover:bg-white/8 transition-colors"
      ...
      <div className="relative z-10 flex flex-1 flex-col overflow-hidden">
        ...
        {/* Toolbar */}
        <header className="flex shrink-0 flex-wrap items-center gap-2 border-b border-[var(--border-subtle)] bg-[var(--bg-elevated)]/80 px-3 py-2.5 md:px-5" style={{ backdropFilter: "blur(12px)" }}>
          {/* Hamburger — mobile only */}
          <button
            onClick={() => setMobileSidebarOpen(true)}
            className="flex md:hidden items-center justify-center w-8 h-8 rounded-md text-[var(--text-tertiary)] hover:text-white hover:bg-white/8 transition-colors"
          ...
          {/* Models row — wraps on small screens */}
          <div className="flex flex-wrap items-center gap-1">
            <span className="text-[10px] font-semibold uppercase tracking-widest text-[var(--text-disabled)] hidden sm:inline">Models</span>
            {ALL_PROVIDERS.map((p) => {
          ...
          {/* Status chip */}
          <span className={`flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[11px] transition-all max-w-[160px] sm:max-w-none truncate ${
            busy
            ...
            {busy && <span className="bd-dot-live shrink-0" style={{ width: 6, height: 6 }} />}
            <span className="truncate">{message || "Ready"}</span>
          </span>
        </header>

        {/* Scrollable body */}
        <main className="flex-1 overflow-y-auto px-3 py-3 md:px-5 md:py-4" style={{ background: "transparent" }}>
